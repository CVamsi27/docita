...
export class DoctorClinicsController {
    ...
    @Post()
    assignDoctor(@Body() data: { doctorId: string; clinicId: string; role?: string }) {
        return this.doctorClinicsService.assignDoctor(data.doctorId, data.clinicId, data.role);
    }
    ...
    @Put('role')
    updateRole(@Body() data: { doctorId: string; clinicId: string; role: string }) {
        return this.doctorClinicsService.updateRole(data.doctorId, data.clinicId, data.role);
    }
}

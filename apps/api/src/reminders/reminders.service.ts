...
Thank you,
{clinicName}`,
                    smsTemplate: 'Reminder: You have an appointment on {appointmentDate} at {appointmentTime} with Dr. {doctorName}. - {clinicName}',
                },
            });
        }
    ...
    async checkAndSendReminders() {
        ...
        const now = new Date();
        const reminderTime = new Date(now.getTime() + settings.hoursBeforeAppt * 60 * 60 * 1000);
    ...
    private async sendEmailReminder(appointment: any, settings: any) {
        try {
            ...
            const message = this.replacePlaceholders(settings.emailTemplate, appointment);
            const subject = this.replacePlaceholders(settings.emailSubject, appointment);
    ...
    private async sendSmsReminder(appointment: any, settings: any) {
        try {
            const message = this.replacePlaceholders(settings.smsTemplate, appointment);
    ...
    private replacePlaceholders(template: string, appointment: any): string {
        ...
        return template
            .replace(/{patientName}/g, `${appointment.patient.firstName} ${appointment.patient.lastName}`)
            .replace(/{doctorName}/g, appointment.doctor.name)
            .replace(/{appointmentDate}/g, date.toLocaleDateString())
            .replace(/{appointmentTime}/g, date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }))

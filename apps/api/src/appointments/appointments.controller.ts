...
export class AppointmentsController {
    ...
    @Post()
    @UsePipes(new ZodValidationPipe(appointmentSchema.omit({ id: true, createdAt: true, updatedAt: true })))
    create(@Body() createAppointmentDto: any) {
        return this.appointmentsService.create(createAppointmentDto);
    }

    @Patch(':id')
    @UsePipes(new ZodValidationPipe(appointmentSchema.partial().omit({ id: true, createdAt: true, updatedAt: true })))

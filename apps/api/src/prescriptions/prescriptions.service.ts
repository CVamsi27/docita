...
export class PrescriptionsService {
    ...
    async generatePDF(id: string): Promise<Buffer> {
        ...
        return new Promise((resolve, reject) => {
            ...
            doc.fontSize(18).font('Helvetica-Bold').text('PRESCRIPTION', { align: 'center' });
            doc.fontSize(10).font('Helvetica').text(`Date: ${new Date(prescription.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}`, { align: 'right' });
            doc.moveDown();
            ...
            doc.fontSize(10).font('Helvetica').text(`Dr. ${prescription.doctor.name}`);
            doc.text(`Email: ${prescription.doctor.email}`);

            // Patient Info (Right)
            doc.fontSize(12).font('Helvetica-Bold').text('Patient:', 350, startY);
            doc.fontSize(10).font('Helvetica').text(`${prescription.patient.firstName} ${prescription.patient.lastName}`);
            doc.text(`Age: ${prescription.patient.dateOfBirth ? new Date().getFullYear() - new Date(prescription.patient.dateOfBirth).getFullYear() : 'N/A'} Years`);
            ...
            if (prescription.instructions) {
                doc.fontSize(12).font('Helvetica-Bold').text('Instructions:', 50, doc.y);

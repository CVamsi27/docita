...
import { Tier, Feature, FEATURE_TIER_MAP, TIER_INFO } from './stores/permission-store'
...
const TierConfigContext = createContext<TierConfigContextType | undefined>(undefined)
...
const DEFAULT_LIMITS: Record<Tier, PlanLimits> = {
  ...
  [Tier.ENTERPRISE]: { patients: 999999, doctors: 999, storageGB: 100, branches: 999 },
  [Tier.INTELLIGENCE]: { patients: 999999, doctors: 999, storageGB: 100, branches: 999 },
}

const DEFAULT_DESCRIPTIONS: Record<Tier, { tagline: string; description: string }> = {
  [Tier.CAPTURE]: { tagline: 'Free forever', description: 'Perfect for getting started with digitization' },
  [Tier.CORE]: { tagline: 'Solo Clinic Essentials', description: 'Essential features for small clinics' },
  [Tier.PLUS]: { tagline: 'WhatsApp Automation', description: 'Advanced features for growing clinics' },
  [Tier.PRO]: { tagline: 'Multi-Doctor Clinics', description: 'Full-featured solution for professional clinics' },
  [Tier.ENTERPRISE]: { tagline: 'Hospital-Grade System', description: 'Hospital-grade solution with full customization' },
  [Tier.INTELLIGENCE]: { tagline: 'AI-Powered Add-on', description: 'Enhance your clinic with AI-powered features' },
}

const DEFAULT_BUNDLE_DISCOUNT = 0.4

export function TierConfigProvider({ children }: { children: React.ReactNode }) {
  ...
  const getPlanPricing = useCallback((tier: Tier): PlanPricing => {
  ...
  const getPlanLimits = useCallback((tier: Tier): PlanLimits => {
  ...
  const getPlanDescription = useCallback((tier: Tier): { tagline: string; description: string } => {
  ...
  const getFeatureDisplay = useCallback((feature: Feature): FeatureDisplay => {
    if (!config?.featureDisplay) {
      return { name: feature, description: '' }
    }
    return config.featureDisplay[feature] ?? { name: feature, description: '' }
  }, [config])
  ...
  const getIntelligenceBundlePrice = useCallback((): number => {
    ...
    const totalPrice = config.intelligenceAddons.reduce((sum, addon) => sum + addon.monthlyPrice, 0)
    return Math.round(totalPrice * (1 - (config.intelligenceBundleDiscount ?? DEFAULT_BUNDLE_DISCOUNT)))
  }, [config])
  ...
  }), [config, isLoading, error, fetchConfig, getPlanPricing, getPlanLimits, getPlanDescription, getFeatureDisplay, getIntelligenceBundlePrice])

...
export default function PatientDetailPage() {
  ...
  const lastVisit = useMemo(() => {
    ...
    return completed.length > 0 ? new Date(completed[0]?.startTime || '') : undefined
  }, [appointments])
  ...
  const nextVisit = useMemo(() => {
    ...
    return upcoming.length > 0 ? new Date(upcoming[0]?.startTime || '') : undefined
  }, [appointments])
  ...
  const handleRepeatPrescription = useCallback((appointmentId: string) => {
    if (patient?.id) {
    ...
    <div className="flex flex-col gap-8 max-w-7xl mx-auto w-full p-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={handleBackToPatients} className="rounded-full hover:bg-muted">
        ...
        <div className="flex gap-3">
          ...
          />
          <Button onClick={handleScheduleVisit} className="rounded-full shadow-lg shadow-primary/20">
      ...
      <Tabs defaultValue="history" className="w-full">
        <div className="flex items-center justify-between mb-6">
          <TabsList className="bg-muted/50 p-1 rounded-full h-12">
            <TabsTrigger value="history" className="rounded-full px-6 h-10 data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <Clock className="mr-2 h-4 w-4" /> Timeline
            </TabsTrigger>
            <TabsTrigger value="documents" className="rounded-full px-6 h-10 data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <FileText className="mr-2 h-4 w-4" /> Documents
            </TabsTrigger>
            <TabsTrigger value="notes" className="rounded-full px-6 h-10 data-[state=active]:bg-background data-[state=active]:shadow-sm">
          ...
          {appointments.length === 0 ? (
            <Card className="border-dashed bg-muted/30">
              <CardContent className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">Schedule a visit to start tracking history.</p>
                <Button variant="link" onClick={handleScheduleVisit} className="mt-2">
                  Schedule Now
                </Button>
              </CardContent>
            </Card>
            ...
            <div className="grid gap-6">
              {appointments.map((apt) => (
                <Card key={apt.id} className="overflow-hidden border-none shadow-sm hover:shadow-md transition-all duration-300 group">
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="bg-primary/5 p-6 md:w-56 flex flex-col justify-center items-center md:items-start border-b md:border-b-0 md:border-r border-border/50 group-hover:bg-primary/10 transition-colors">
                      ...
                      <Badge variant={apt.status === "completed" ? "default" : "secondary"} className="mt-4 w-full justify-center capitalize rounded-full">
                        {apt.status}
                      </Badge>
                    </div>
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-between gap-6">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {apt.status === "completed" && apt.prescription && apt.id && (
                            <Button
                              size="sm"
                              variant="secondary"
                              className="rounded-full gap-1"
                              onClick={() => handleRepeatPrescription(apt.id!)}
                            >
                        ...
                        <div className="bg-muted/30 rounded-xl p-4 space-y-2">
                          ...
                          {apt.vitalSign ? (
                            <div className="text-sm font-medium">
                              {apt.vitalSign.bloodPressure && <div className="flex justify-between"><span>BP</span> <span>{apt.vitalSign.bloodPressure}</span></div>}
                              {apt.vitalSign.pulse && <div className="flex justify-between"><span>HR</span> <span>{apt.vitalSign.pulse} bpm</span></div>}
                              {apt.vitalSign.temperature && <div className="flex justify-between"><span>Temp</span> <span>{apt.vitalSign.temperature}°F</span></div>}
                            </div>
                          ) : (
                            <div className="text-sm text-muted-foreground italic">No vitals recorded</div>
                          )}
                        </div>
                        ...
                        <div className="bg-muted/30 rounded-xl p-4 space-y-2">
                          ...
                          {apt.prescription ? (
                            <div className="space-y-2">
                              <div className="text-sm font-medium">
                                {apt.prescription.medications?.length || 0} Medications
                              </div>
                              <Button variant="link" className="h-auto p-0 text-xs" onClick={() => router.push(`/prescriptions/${apt.prescription?.id}`)}>
                                View Prescription
                              </Button>
                            </div>
                          ) : (
                            <div className="text-sm text-muted-foreground italic">No prescription</div>
                          )}
                        </div>
        ...
        <TabsContent value="documents" className="mt-6">
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-4">
            {documents.map((doc) => (
              <Card key={doc.id} className="group cursor-pointer hover:shadow-md transition-all border-none bg-muted/30 hover:bg-background">
                <CardContent className="p-6 flex flex-col items-center text-center gap-4">
                  <div className="p-4 bg-primary/10 rounded-full text-primary group-hover:scale-110 transition-transform">
                    <FileText className="h-8 w-8" />
                  </div>
                  <div>
                    <h4 className="font-semibold truncate w-full px-2">{doc.name}</h4>

...
export function middleware(request: NextRequest) {
    const token = request.cookies.get('docita_token')?.value || request.headers.get('Authorization')?.split(' ')[1]
    ...
    const isPublicPath = request.nextUrl.pathname.startsWith('/_next') || request.nextUrl.pathname.startsWith('/api') || request.nextUrl.pathname.startsWith('/static') || request.nextUrl.pathname === '/favicon.ico'

...
import { Feature, Tier, getTierInfo, FEATURE_TIER_MAP } from '@/lib/tier-config'
  ...
  return (
    <div className={cn(
      ...
      <p className="text-muted-foreground text-center mb-4 max-w-md">
        This feature requires <span className="font-medium text-foreground">{tierInfo.name}</span>.
...
function TierUpgradePrompt({ tier, className }: { tier: Tier; className?: string }) {
  const tierInfo = getTierInfo(tier)
 ...
 */
export function FeatureTierBadge({ feature, className }: { feature: Feature; className?: string }) {
  const requiredTier = FEATURE_TIER_MAP[feature]

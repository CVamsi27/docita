...
export type TemplatesSettingsProps = object;
...
export function TemplatesSettings() {
  ...
  const specialtyOptions = useFormOptions("specialty");
  const templateFieldTypeOptions = useFormOptions("templateFieldType");
            ...
            <div className="space-y-4">
              ...
              {formData.fields?.length === 0 && (
                <div className="text-center p-8 border-2 border-dashed rounded-lg text-muted-foreground text-sm">
                  No custom fields added. The standard fields (Vitals,
                  Diagnosis) will still appear.
                </div>
              )}
              ...
              {formData.fields?.map((field, idx) => (
                <div
                  key={idx}
                  className="flex gap-3 items-start p-3 bg-muted/30 rounded-md border"
                >

...
export function CptCodeSearch({ onSelect, selectedCodes = [] }: CptCodeSearchProps) {
  ...
  const loadFavorites = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setFavorites(data.map((fav: any) => fav.cptCode))
      }
    } catch (error) {
  ...
  React.useEffect(() => {
    async function searchCodes() {
      ...
      setLoading(true)
      try {
        const res = await fetch(`${API_URL}/medical-coding/cpt-codes?search=${encodeURIComponent(debouncedQuery)}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <span className="text-muted-foreground">Search CPT codes (e.g. &quot;99213&quot; or &quot;Office visit&quot;)...</span>

  ...
  tiers: [
    {
      ...
      features: ['Up to 100 patients', 'Basic patient records', 'Excel/PDF import', 'Manual WhatsApp share'],
    },
    {
      id: 'CORE',
      tier: 1,
      ...
      features: ['500 patients', 'Calendar & scheduling', 'Digital prescriptions', 'WhatsApp integration', 'ICD-10 medical coding', 'Basic analytics'],
    },
    {
      id: 'PLUS',
      tier: 2,
      ...
      features: ['2,000 patients', 'Everything in Core', 'Prescription templates', 'Custom form fields', 'Clinic branding', 'Priority support'],
    },
    {
      id: 'PRO',
      tier: 3,
      ...
      features: ['10,000 patients', 'Everything in Plus', 'Up to 5 doctors', 'Advanced analytics', 'Revenue reports', 'API access'],
    },
    {
      id: 'ENTERPRISE',
      tier: 4,
      ...
      features: ['Unlimited patients', 'Everything in Pro', 'Unlimited doctors', 'Multi-branch support', 'Custom integrations', 'Dedicated support'],
    },
  ],
  featureTierMap: {},
  featureDisplay: {},
...
export function useTierConfig() {
  ...
  const getIntelligenceBundlePrice = useCallback((): number => {
    const totalPrice = config.intelligenceAddons.reduce((sum: number, addon: IntelligenceAddon) => sum + addon.monthlyPrice, 0)
  ...
  const getPricing = useCallback(
    (tierId: string, currency: string): { monthly: number; yearly: number; isCustom: boolean } | null => {
